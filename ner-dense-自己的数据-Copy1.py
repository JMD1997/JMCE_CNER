...
batch_size = 24
bert_layers = 12
learning_rate = 3e-4  # bert_layers越小，学习率应该要越大
...
import pypinyin
# tmpdf
tmpdf["全拼"]=tmpdf["汉字"].apply(lambda x:pypinyin.pinyin(x)[0][0])
...
def get_embed(d_rad2_index,d_rad):
    sentence_rad=[[d_rad[i] for i in j if i in d_rad] for j in sentence]

    import pandas as pd
    import gensim
    w2v_model_rad = gensim.models.Word2Vec(sentence_rad, size=32, iter=10, min_count=0)
    word_vectors_rad = w2v_model_rad.wv


    embedding_matrix_rad = np.zeros((len(d_rad2_index.keys()) + 3, 32), dtype=np.float32)
    not_in_model = 0
    in_model = 0
    ...
    for word, i in d_rad2_index.items():
        if word in word_vectors_rad:
            in_model += 1
            embedding_matrix_rad[i] = np.array(w2v_model_rad[word])
        else:
            not_in_model += 1
            not_words.append(word)
    return embedding_matrix_rad


# In[15]:


embedding_matrix_rad=get_embed(d_rad2_index,d_rad)
...
embedding_matrix_pin=get_embed(d_pin2_index,d_pin)
...
embedding_matrix_bi=get_embed(d_wubi2_index,d_wubi)
...
def get_rad_embed():
    embed = Embedding(len(d_rad2_index)+3, 32, weights=[embedding_matrix_rad],trainable=True,mask_zero=True)  #定义一个词嵌入层,将句子转化成对应的向量
...
def get_pinyin_embed():
    embed = Embedding(len(d_pin2_index)+3, 32,weights=[embedding_matrix_pin],trainable=True,mask_zero=True)  #定义一个词嵌入层,将句子转化成对应的向量
...
def get_wubi_embed():
    embed = Embedding(len(d_wubi2_index)+3, 32,weights=[embedding_matrix_bi],trainable=True,mask_zero=True)  #定义一个词嵌入层,将句子转化成对应的向量
    input_wubi=Input(shape=(None,))
    emb_wubi=embed(input_wubi)
    #outwubi=Dense(16)(emb_wubi)
    outwubi=Bidirectional(LSTM(32,return_sequences=True))(emb_wubi)
...
# loss图和the accuracy of iteration
...
loss = history.history['sparse_accuracy']
epochs = range(1, len(loss ) + 1)
...
for i in test:
    try:
        l.append(NER.recognize(i.strip())) 
    except:
        l.append("")
...
dftest.to_csv("test_res.csv",header=True,index=False,encoding="utf-8-sig")
